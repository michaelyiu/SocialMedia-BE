...
export default {
  Mutation: {
    createEducation: async (parent, args, { me, models }, info) => {
    ...
    editEducation: async (parent, args, { me, models }, info) => {
      const { errors, isValid } = validateEducationInput(args);
      ...
      const eduFields: { school?: string, degree?: string, fieldOfStudy?: string, from?: string, to?: string, current?: string, description?: string } = {};
    ...
    deleteEducation: async (parent, args, { me, models }, info) => {
      args.user_id = me.id;
