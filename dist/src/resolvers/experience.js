...
export default {
    Mutation: {
        createExperience: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
        ...
        editExperience: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
            const { errors, isValid } = validateExperienceInput(args);
            if (!isValid) {
                throw new GraphQLError("Some required fields should not be empty!", {
                    extensions: {
            ...
            if (args.from || args.from === "")
                expFields.from = args.from;
            if (args.to || args.to === "")
                expFields.to = args.to;
            ...
            const newProfile = await models.Profile.findOneAndUpdate({ user: me.id }, { $set: { [`experience.${index}`]: expFields } }, { new: true })
                .then(profile => {
                return profile;
            })
                .catch(err => {
        ...
        deleteExperience: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
            args.user_id = me.id;
