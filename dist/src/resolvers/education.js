...
export default {
    Mutation: {
        createEducation: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
        ...
        editEducation: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
            const { errors, isValid } = validateEducationInput(args);
            if (!isValid) {
                throw new GraphQLError("Some required fields should not be empty!", {
                    extensions: {
            ...
            args.user_id = me.id;
            const eduFields = {};
            if (args.school || args.school === "")
                eduFields.school = args.school;
            if (args.degree || args.degree === "")
                eduFields.degree = args.degree;
            if (args.fieldOfStudy || args.fieldOfStudy === "")
                eduFields.fieldOfStudy = args.fieldOfStudy;
            if (args.from || args.from === "")
                eduFields.from = args.from;
            if (args.to || args.to === "")
                eduFields.to = args.to;
            ...
            const newProfile = await models.Profile.findOneAndUpdate({ user: me.id }, { $set: { [`education.${index}`]: eduFields } }, { new: true })
                .then(profile => {
                return profile;
            })
                .catch(err => {
        ...
        deleteEducation: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
            args.user_id = me.id;
