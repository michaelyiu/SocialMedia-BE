...
export default {
    Query: {
        ...
        profile: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
    ...
    Mutation: {
        updateProfile: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
            ...
            if (args.handle)
                profileFields.handle = args.handle;
            ...
            const updatedProfile = await models.Profile.findOne({ user: me.id })
                .then(async (profile) => {
                if (profile) {
                    return await models.Profile.findOneAndUpdate({ user: me.id }, { $set: profileFields }, { new: true }).then(profile);
                }
                else {
                    //Create
        ...
        deleteProfile: combineResolvers(isAuthenticated, async (parent, args, { me, models }, info) => {
